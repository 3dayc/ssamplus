<template lang="html">
  <div>
    <div class="infoCompetitionChild">
      <div class="kind">
        <div class="row">
          <span class="selectSet">
            <select name="" id="" @change="onChange2($event)">
              <option value="">학년도</option>
              <option value="year00" v-if="!this.mobile" selected>최근 3년</option>
          ...
          <span class="selectSet">
            <select name="" id="">
              <option value="">정시/추시</option>
              <option value="" selected>정시</option>
              <option value="">추시</option>
            </select>
          </span>
                    <span class="selectSet">
            <select name="" id="" @change="onChange($event)">
              <option value="">구분</option>
              <option value="type01" selected>일반</option>
              <option value="type02">장애</option>
              <option value="type03">지역</option>
              <option value="type04">IB학교</option>
            </select>
          </span>
          <!-- <multiselect 
      ...
      <div v-if="!this.mobile">
        <div class="conditionTabWrap">
          <h4><strong>최근 3년, 정시, 일반, 유아!</strong> 몇 명을 모집하고 몇 명이 지원했을까?</h4>
          <!-- tab -->
          <div class="tabBack">
            <div class="tabNav">
              <nav>
                <ul>
                  <li :class="{on:currentTab===0}"><button @click="currentTab = 0">사전예고인원 대비 모집인원</button></li>
                  <li :class="{on:currentTab===1}"><button @click="currentTab = 1">모집인원 대비 지원인원</button></li>
                  <li :class="{on:currentTab===2}"><button @click="currentTab = 2">경쟁률</button></li>
                  <li :class="{on:currentTab===3}"><button @click="currentTab = 3">1차, 최종합격선</button></li>
                </ul>
              </nav>
            </div>
          </div>
          <!--// tab -->
          <div class="tabLayout">
            <div v-if="currentTab == 0" class="tabData case1" :key="currentTab">
              <InfoGraphRecruitment />
            </div>
            <div v-if="currentTab == 1" class="tabData case2">
              <InfoGraphApplicants />
            </div>
            <div v-if="currentTab == 2" class="tabData case3">
              <InfoGraphCompete />
            </div>
            <div v-if="currentTab == 3" class="tabData case4">
              <InfoGraphPass />
            </div>
          </div>
        </div>
      ...
      <div v-else-if="this.mobile">
        <div class="mGraphBack">
          <h4><strong>최근 3년, 정시, 일반, 유아!</strong> 몇 명을 모집하고 몇 명이 지원했을까?</h4>
          <div class="mGraph">
            <strong>사전예고인원 대비 모집인원 (일반기준)</strong>
            <InfoGraphRecruitment />
          </div>
          <div class="mGraph">
            <strong>모집인원 대비 지원인원 (일반기준)</strong>
            <InfoGraphApplicants />
          </div>
          <div class="mGraph">
            <strong>경쟁률 (일반기준)</strong>
            <InfoGraphCompete />
          </div>
          <div class="mGraph">
            <strong>1차, 최종합격선 (일반기준)</strong>
            <InfoGraphPass />
          </div>
        </div>
      </div>
      <!-- //모바일 용 -->
      <!-- <InfoFullDataTableKind0 /> -->
      <h4>
        <strong>최근 3년, 정시, 일반, 유아!</strong>의 최근 3개년 경쟁률 및 합격선
      </h4>
...
import InfoGraphRecruitment from '@/views/exam/InfoGraphRecruitment.vue';
import InfoGraphApplicants from '@/views/exam/InfoGraphApplicants.vue';
import InfoGraphCompete from '@/views/exam/InfoGraphCompete.vue';
import InfoGraphPass from '@/views/exam/InfoGraphPass.vue';

export default {
  mixins: [Mixin],
  name: 'InfoCompetitionChild',
  components: {
    InfoGraphRecruitment,
    InfoGraphApplicants,
    InfoGraphCompete,
  ...
  methods: {
    ...
    onChange2(event) {
      ...
      else if (value == 'year01') this.year = 3;
      else if (value == 'year02') this.year = 2;
      else if (value == 'year03') this.year = 1;
    },
  },
  mounted() {
    this.listLoad();
